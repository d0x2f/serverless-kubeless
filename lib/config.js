...
class Config {
  constructor(options) {
    const opts = _.defaults({}, options, {
      namespace: process.env.KUBELESS_NAMESPACE || 'kubeless',
      deploymentRetryLimit: parseInt(process.env.DEPLOYMENT_RETRY_LIMIT, 10) || 3,
      deploymentRetryInterval: parseInt(process.env.DEPLOYMENT_RETRY_INTERVAL, 10) || 2,
    });
    this.namespace = opts.namespace;
    this.deploymentRetryLimit = opts.deploymentRetryLimit;
    this.deploymentRetryInterval = opts.deploymentRetryInterval;
