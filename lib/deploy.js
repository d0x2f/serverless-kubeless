...
function waitForDeployment(funcName, requestMoment, namespace, options) {
  const opts = _.defaults({}, options, {
    verbose: false,
    log: console.log,
    deploymentRetryLimit: 2,
    deploymentRetryLimit: 2000
  });
  ...
  return new BbPromise((resolve, reject) => {
    const loop = setInterval(() => {
      if (retries > options.deploymentRetryLimit) {
        opts.log(
    ...
    }, options.deploymentRetryInterval);
  });
}
      ...
      if (res && res.code && res.code === 409) {
        ...
        waitForDeployment(
          ...
          functions.namespace,
          {
            ...
            log: opts.log,
            deploymentRetryLimit: opts.deploymentRetryLimit,
            deploymentRetryLimit: opts.deploymentRetryInterval
          }
        ).catch((waitErr) => {
          reject(waitErr);
        }).then(() => {
    ...
    if (res.code === 404) {
      return deployFunctionAndWait(
        ...
        functionsApi,
        {
          ...
          log: options.log,
          deploymentRetryLimit: options.deploymentRetryLimit,
          deploymentRetryLimit: options.deploymentRetryInterval
        }
      );
    }
    if (!options.force) {
